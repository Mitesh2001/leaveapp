...
class Menu
{
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        // if(isset($item['only_owner_access'])) {
        //     if(!auth()->user()->roles->first()->user_type == 0 && $item['only_owner_access'] = 1) {
        //         if (!auth()->user()->can('leave-types-view') || !auth()->user()->can('role-view')) {
        //             return false;
        //         }
        //     }
        // }
        ...
        if(isset($item['only_owner_access'])) {
            ...
            if (!(auth()->user()->can('leave-types-view') || auth()->user()->can('holidays-view') || auth()->user()->can('role-view')) && $item['only_owner_access'] = 1) {
                return false;
            }
